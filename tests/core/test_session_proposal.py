...
def test_proposal_revise_records_human_review(tmp_path):
    init_project(tmp_path / "p", "P")
    paths = ProjectPaths(tmp_path / "p")
    now = datetime.now(timezone.utc)
    session = create_session(paths, title="Intake", mode="intake", created_at=now)
    ...
    updated = set_proposal_action(paths, proposal.id, action="revise", note="needs split", reviewed_at=now)
    assert updated.state.value == "revised"
    assert updated.human_review["action"] == "revise"
